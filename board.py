...
class Board:
    ...
    def move_build(self, worker, move_direction, build_direction):
        ...
        new_row, new_col, old_row, old_col = worker.update_workers_location(move_direction)
        #get the coordinates of the worker's new cell
        # new_row = worker._my_row
        # new_col = worker._my_col
        new_cell = self._matrix[new_row][new_col]

        # new_row, new_col = worker.update_workers_height(new_cell._height) #update the worker's stored height

        #Reflect changes on the board:
        #update the current cell to None
        previous_cell = self._matrix[old_row][old_col]
        previous_cell._worker = None

        # Finish moving worker to the new cell
        new_cell._worker = worker          #If I used a set_worker_on_cell(), it would be new_cell.set_worker_on_cell(changed_worker)
